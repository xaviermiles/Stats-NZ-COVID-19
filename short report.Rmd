...
After importing the data from the CSV file, I changed the _class_, _category_, and _indicator_name_ columns into factor type, since these are categorical variables. I changed the _date_last_updated_ column into Date type, since there was consistent formatting for this column.
...
I chose data that seemed relatively objective and included information from before the COVID-19 NZ response began, so that I could compare before and after to generate insights into changes due to COVID-19. The data was also chosen to provide insight into the wellbeing of New Zealanders.
...
```{r jobseeker, figures-side, fig.show="hold", out.width="50%", echo=FALSE}
